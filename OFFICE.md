...
- Six procedural core-team characters: Nora, Maya, Sam, Ava, Theo and Noor. Characters have distinct appearances, seated poses, small work/idle gestures, and articulated walking cycles.
- Drag to orbit; scroll or use the camera buttons to zoom. Room navigation focuses the camera. **O** returns to the overview.
- Projects opens first after sign-in. The Office view initializes only when opened and stops rendering while another screen is visible. Select another character in the scene or team list to open their **Chat**, **Tasks**, **Review**, and **Profile** panel. The panel docks on wide desktops and overlays the office on tablets. **Escape** closes the selection.
- **Departments & skills** opens a searchable directory of 20 profiles across Leadership, Product, Design, Engineering, QA and Platform. Each profile lists responsibilities, deliverables, boundaries, assigned skills and pinned source links. The sidebar shows the six core chats first, with fourteen specialists in a collapsed group. All agent IDs and saved conversations are preserved. Tool availability is separate from team membership.
- Chat directly with Nora, Maya, Sam, Ava, Theo and Noor, with separate histories, live replies, queued/working states and stop/retry controls. Save tasks for each agent, with separate conversations for each task. Sam's local Codex worker handles real assignments against Virtual-Team, asks questions in chat, edits an isolated branch, runs checks and prepares code for review. The queue and team presence show actual worker activity. Project document and review roles execute through a project’s **Work & reviews**; their general conversations and standalone task queues remain separate. Records persist in PostgreSQL through authenticated Vercel API handlers. Only confirmed responses count as saved; failures retain the draft and provide retry controls. The visible office synchronizes worker updates automatically.
...
- **Call a meeting** sends the six seated avatars directly through the hall, carrying closed laptops. They use the side entrances, clear the board, and follow the table's side aisles to their seats. Attendance updates as characters arrive; each person places and opens their laptop. **End meeting** closes and packs the laptops before everyone returns through those entrances to their original places.
...
This is the office and interaction layer with real cloud persistence and one shared local worker for six core chats, Sam coding assignments and connected project stages. All general chat is read-only; Sam assignments authorize isolated code editing. His availability depends on the owner's computer and worker. All 20 agent role definitions and skill assignments are configured in the roster. Seven additional roles execute project documents and independent document reviews; Figma, project frontend and browser QA remain capability-blocked. Desk displays, room occupancy and meeting attendance remain illustrative local animation. No microphone or voice meeting runtime is connected. See RUNTIME.md for worker setup and boundaries.
...
The local rebuild is not deployed; do not push without explicit owner authorization. The **Projects & approvals** view contains kickoff, the ordered handoffs, versioned documents, independent reviews, owner decisions, questions and project conversation/history. It reflects persisted worker activity. See PROJECT_WORKFLOW.md.

## Source

- `agent-library/`: 20 role definitions, generated briefs, 25 pinned skill packages, source metadata, licenses and integrity hashes.
- `AGENT_TEAM.md`: generated department and skill map.
- `src/office/team-directory.js` and `team-directory.css`: searchable department directory and agent skill profiles.
- `src/office/config.js`: merges the roster with existing room, avatar and meeting-seat coordinates; exports the full roster separately from the six physical avatars.
...
The workspace now has Projects, Office and Settings navigation. Project dashboards, draft creation, references, work/review entrypoints and project settings are documented in [WORKSPACE_UX.md](WORKSPACE_UX.md). Department and skill browsing is embedded in Settings. Workspace chats retain their existing conversation scopes; project-common chat remains parked.
