...
- Save messages and queue tasks for each agent, with separate conversations for each task. Task history and the office work queue show status, assignment, and items needing your attention. These records persist in PostgreSQL through authenticated Vercel API handlers. Only confirmed responses count as saved; failures retain the draft and provide retry controls. Refreshing or returning to the browser reloads the latest workspace.
...
This is the office and interaction layer with real cloud persistence. Names, roles, desk displays, activity, and meeting attendance are illustrative local prototype state. No agents execute code, no work is monitored, no microphone is used, and no live agent or voice meeting runtime is connected. Real tasks stay **Queued**, and saved messages do not receive generated agent replies. The interface identifies agents as **Not connected** and the office as a preview.

Completed sample reviews are saved decisions, not merges or deployments. Everyone who knows the access code uses the same private workspace. Chats and tasks survive clearing browser data, although signing in again is required. Unsaved composer drafts are retained while switching agents and tabs in the current page and are cleared on signout or session expiry. This is not a multi-user permissions system. Earlier browser-local prototype records are not automatically uploaded.
