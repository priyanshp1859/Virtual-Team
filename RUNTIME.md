...
The Vercel office and Neon database are the shared interface and durable queue. A Node.js worker on the owner's computer starts Codex App Server over private stdio. The six core agents use the existing ChatGPT Codex login; no model credential is sent to Vercel or the browser. This first worker is verified on macOS with Node 22 and Codex CLI 0.154.0.

## Workflow

- General chat for Nora, Maya, Sam, Ava, Theo and Noor answers questions using each role’s own instructions, skills and conversation history. It can read the isolated Virtual-Team repository. General chat cannot edit files, approve stages, contact another agent or execute project handoffs. Create a Sam assignment to authorize code edits.
- New real tasks assigned to Sam are queued automatically. Earlier saved tasks have a Start Sam control. Other roles’ standalone tasks remain saved without execution; their eligible project stages run through Projects.
...
- Codex’s `ask_user` tool writes a question into the same conversation. For general chat, the turn ends and releases the shared worker. An answer tied to that exact question queues a new turn with the saved history; it does not hold up other chats. Sam coding assignments retain their in-turn question/answer flow.
...
The office shows per-chat queued, working, replied, waiting, error and cancelled states. Connected roles require this computer to be awake, online and running the worker; the website and saved data remain available when it is off. Restart the worker after changing its code, configuration or Codex installation. Existing tasks are retained.
...
Code tasks, chat turns and project stages share one serial worker. Sending a follow-up while a reply is running queues a later turn, and message IDs distinguish inputs already answered from new inputs. Each core general conversation has its own run key and isolated clone. General chat is read-only. The worker can use already installed dependencies, but adding dependencies, external services, binary artifacts and broad changes require later workflow support. Reviews are limited to 24 text files and 140 KB of before/after content; oversized results fail visibly and are retained locally. Failed test/build checks block publication. The live agent data has a bounded storage limit; existing records are preserved when full.
