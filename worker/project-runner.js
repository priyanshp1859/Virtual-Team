...
export async function runProjectJob({ sql, owner, job, repository, stateDir, baseRef, settings, isStopped, setActiveClient }) {
  ...
  try {
    ...
    const context = { title: job.project.title, brief: job.project.brief, referenceDocument: job.project.sourceText, resources: (job.project.resources || []).map(r => r.readable ? { name: r.name, text: r.text } : { name: r.name, status: 'Stored reference only; binary content is not accessible to this agent.' }), figmaUrl: job.project.figmaUrl, designSystemUrl: job.project.designSystemUrl, stage: d, feedback: job.project.steps[d.id].feedback || '', artifacts: job.project.artifacts.filter(a => claim.inputs.some(i => i.id === a.id) || a.stepId === d.id).slice(-12), recentConversation: job.project.messages.slice(-16) };
