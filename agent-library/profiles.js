...
import { WORKFLOW_STEPS, WORKFLOW_CAPABILITIES } from '../src/office/workflow-config.js';
...
export function profileMarkdown(agentId) {
  const agent = getProfile(agentId);
  return `# ${agent.name} — ${agent.role}\n\nStatus: ${agentId === 'sam' ? 'Connected through the existing local worker when it is online.' : WORKFLOW_STEPS.some(step => step.agentId === agentId && WORKFLOW_CAPABILITIES[step.kind]) ? 'Connected for assigned project documents and reviews when the local worker is online. General chat is saved separately; use Projects for live work.' : 'Profile configured. Required execution capabilities are not connected.'}\n\n${roleInstructions(agentId)}\n\n## Skill provenance\n\n${agent.skills.map(binding => {
    const skill = catalog.skills.find(item => item.id === binding.id);
